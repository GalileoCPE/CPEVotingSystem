...
// Monitoring du nombre de votes
pm2.metric({
    type: 'metric',
    name: 'Votes count',
    value: function () {
        return Object.keys(VW.votes).length;
    }
});
...
exports.VW = function () {
    ...
    for (let i = 0; i < vIDs.length; i++) {
        ...
        if (cID in vw.candidates) {
            vw.candidates[cID].votes++;
        } // On incrémente le nombre de votes
    }
    Object.entries(vw.candidates).sort((x, y) => y[1].votes - x[1].votes).forEach((x, idx) => x[1].rank = idx + 1)
